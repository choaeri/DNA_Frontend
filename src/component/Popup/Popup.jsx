...
export default function Popup() {
  ...
  useEffect(() => {
    if (isLoggedIn && popupCheck) {
      const fetchPopupStatus = async () => {
        setIsPopup(true);
        processOffPopupCheck();
        ...
        try {
          const res = await axiosInstance.get("/api/workation-schedules/expired-no-review");
  ...
  return (
    <>
      <Modal open={isPopup} onClose={() => setIsPopup(false)}>
        <section className="popupSection">
          <div className="popupModal">
            ...
            <div className="btnCnt">
              <button className="writeBtn" onClick={() => navigate("/mypage/schedule")}>
                <span>Go to write a review</span>
              </button>
            </div>
          </div>
        </section>
