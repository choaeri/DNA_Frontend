...
export default function DetailLocation() {
	...
	useEffect(() => {
		...
		const fetchFacilitiesCount = async () => {
			try {
				const res = await axiosInstance.get(
					`/api/public/locations/${locationId}/facilities/count`
				);
				setCount(res.data);
			} catch (err) {
				errMessageCheck(err.response.data.errorMessage);
	...
	const handleImageClick = () => {
		...
		} else if (imageCntOpen) {
			setImageCntOpen(false);
		}
	};
		...
		<div className="DetailLocation">
			...
			<div className="imageContent" onClick={handleImageClick}>
				...
				<svg
					style={
						!imageCntOpen
							? { transform: 'rotate(0deg)', transition: 'all 0.3s ease-out' }
				...
				>
					<path
						d="M14.5 4.70996L14.5 22.0582"
						stroke="black"
						strokeWidth="1.5"
					/>
					<path
						d="M22.2104 15L14.5001 22.7103L6.78983 15"
						stroke="black"
						strokeWidth="1.5"
					/>
				</svg>
			...
			>
				{imageCntOpen && (
					<LocationImage locationName={detailInfo.locationName} />
				)}
			</div>
		</div>
	);
