...
export default function ListView() {
	const { locations, facilityCount, setSelectLocationName } =
					...
					<div key={index} className="card">
						...
						<div className="locationCnt">
							<div className="header">
								<span className="location">{locationMatch[location.locationName]}</span>
								<svg
									className="modalBtn"
								...
								>
									...
									<path
										d="M15.2898 6.5L23.0001 14.2103L15.2898 21.9206"
										stroke="black"
									/>
								</svg>
							</div>
								...
								<div className="item spots">
									...
									{facilityCount?.map((count, idx) => {
										return count.locationId === location.locationId ? (
											<span key={idx} className="detailCtn spot">{count.facilityCount} spots</span>
										) : null;
									})}
								</div>
								<div className="item desp">
									<span className="detailCtn desp">{location.description}</span>
								</div>
							</div>
						</div>
					</div>
