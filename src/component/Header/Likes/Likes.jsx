    ...
    <div className="Likes">
      ...
      <div className="bmkCnt">
        ...
        {bookmarkList && Object.keys(bookmarkList).length > 0 ? (
                ...
                {bookmarkList[locationName].map((bookmark, idx) => {
                  return bookmark.type === "workation" ? (
                    <div className="item" key={idx}>
                      ...
                      <WorkationDetail locationName={bookmark.locationName} facilityName={bookmark.facilityName} facilityId={bookmark.facilityId} />
                    </div>
                  ) : null;
                })}
              </div>
          ...
          <p>북마크가 없습니다.</p>  // bookmarkList가 없을 때 표시할 내용
        )}
      </div>
    </div>
  );  
